...
export async function importDbEntries(): Promise<IClaim[]> {
    ...
    arr.filter(claim => claim.CFDocument.creator !== 'Smarter Balanced_Math Claim 2-4 Item Specifications').forEach((claim, idx) => {
        newClaim = {};
        ...
        if (claim.CFItems[0].educationLevel.length > 1) {
            claim.CFItems[0].educationLevel.forEach((edu: string) => newClaim.grades.push(`${parseInt(edu, 10)}`));
        }
        else {
        ...
        catPT.filter((p: string) => p.includes('CAT')).forEach((p: string) => newClaim.target[0].type = p);
...
export function getClaimShortCode(subject: string, claim: string, grade: string[]) {
    ...
    if (grade.length > 1) {
        if (parseInt(grade[0], 10) < 9) {
            ...
            return (subject === Subject.ELA) ? `E.G${grade[0]}-${grade[grade.length - 1]}.${claim}` : `M.G${grade[0]}-${grade[grade.length - 1]}.${claim}`;
        }
    }
...
function getClaimDomain(subject: string, shortCode: string, ELASpec: ISpecDocument): string | undefined {
...
function getClaimDesc(subject: string, shortCode: string, ELASpec: ISpecDocument, MATHSpec: ISpecDocument): string | undefined {
